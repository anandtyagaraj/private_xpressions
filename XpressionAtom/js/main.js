...
function renderBattery(battery) {
	...
	context.arc(canvas.width * 3.3 / 10, canvas.height * 5 / 10, outerradius, 1.65 * Math.PI,
			Math.PI * 0.35, false);
	context.stroke();
...
function renderDigitalTime(hour, minute, day, dateOfMonth, month, seconds) {
	...
	context.beginPath();
	context.lineJoin = "round";
	context.moveTo(canvas.width * 3.7 / 10, canvas.height * 4.1 / 10);
	context.lineTo(canvas.width * 8.9 / 10, canvas.height * 4.1 / 10);
	context.stroke();
	...
	if (timeformat === "0") {
		context.fillText(hour + ":" + minute, canvas.width * 6.5 / 10,
				canvas.height / 2);
		context.font = '20px monospace';
		context.fillStyle = colorSec;// '#71E1F9';
		context.fillText(seconds, canvas.width * 8.3 / 10,
				canvas.height * 5.1 / 10);

		context.font = '15px monospace';
		context.fillText(ampm, canvas.width * 4.6 / 10,
	...
	context.beginPath();
	context.lineJoin = "round";
	context.moveTo(canvas.width * 3.7 / 10, canvas.height * 5.9 / 10);
	context.lineTo(canvas.width * 8.9 / 10, canvas.height * 5.9 / 10);
	context.stroke();
	...
	try {
		if (day === 0) {
			sDay = LANG_JSON_DAY_LONG_DATA["Sunday"];
		} else if (day === 1) {
			sDay = LANG_JSON_DAY_LONG_DATA["Monday"];
		} else if (day === 2) {
			sDay = LANG_JSON_DAY_LONG_DATA["Tuesday"];
		} else if (day === 3) {
			sDay = LANG_JSON_DAY_LONG_DATA["Wednesday"];
		} else if (day === 4) {
			sDay = LANG_JSON_DAY_LONG_DATA["Thursday"];
		} else if (day === 5) {
			sDay = LANG_JSON_DAY_LONG_DATA["Friday"];
		} else if (day === 6) {
			sDay = LANG_JSON_DAY_LONG_DATA["Saturday"];
		}
	} catch (Exception) {
...
/*function onlocaleSuccessCallback(locale) {
	...
	try {
		...
		canvas.addEventListener('click', function(e) {
			...
			try {
				...
				if(x > canvas.width * 2/10 && x <= canvas.width *4/10 &&
						y > canvas.height * 4/10 && y <= canvas.height * 6/10){
